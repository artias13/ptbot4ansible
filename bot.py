...
def findPhoneNumbers(update: Update, context):
    ...
    phoneNumRegex = re.compile(
        r"(\+7|\b8)(-|\s)?(\(\d{3}\)|\d{3})(-|\s)?(\d{3})(-|\s)?(\d{2})(-|\s)?(\d{2})"
    )
    ...
    update.message.reply_text(
        "Хотите ли вы сохранить эти номера в базе данных? (yes/no)"
    )
...
def findEmails(update: Update, context):
    ...
    # Завершаем работу обработчика диалога
    update.message.reply_text(
        "Хотите ли вы сохранить эти email адреса в базе данных? (yes/no)"
    )
...
def savePhoneNumber(update: Update, context):
    logger.info(
        f"Пользователь {update.message.from_user.username} выбрал сохранить номер телефона"
    )
    ...
    try:
        ...
        for phone in saved_phones:
            insert_query = (
                f"INSERT INTO phone_numbers (phone_number) VALUES ('{phone}')"
            )
        ...
        logger.info(
            f"{len(saved_phones)} телефонных номеров успешно сохранены в базе данных"
        )
        update.message.reply_text(
            f"{len(saved_phones)} телефонных номеров успешно сохранены в базе данных"
        )
        ...
        update.message.reply_text(
            "Ошибка при сохранении телефонного номера в базу данных"
        )
...
def saveEmailAddress(update: Update, context):
    logger.info(
        f"Пользователь {update.message.from_user.username} выбрал сохранить email адрес"
    )
    ...
    try:
        ...
        for email in saved_emails:
            insert_query = (
                f"INSERT INTO email_addresses (email_address) VALUES ('{email}')"
            )
            insert_queries.append(insert_query)
        ...
        execute_query(";".join(insert_queries), "insert")
        logger.info(
            f"{len(saved_emails)} email адресов успешно сохранены в базе данных"
        )
        update.message.reply_text(
            f"{len(saved_emails)} email адресов успешно сохранены в базе данных"
        )
...
def connectToHost(command):
    ...
    try:
        ...
        client.connect(
            hostname=RM_HOST, username=RM_USER, password=RM_PASSWORD, port=RM_PORT
        )
...
def getDf(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о состоянии файловой системы"
        )
...
def getFree(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о состоянии оперативной памяти"
        )
...
def getMpstat(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о производительности системы"
        )
...
def getW(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о работающих в системе пользователях"
        )
...
def getAuths(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о последних 10 входах"
        )
...
def getCritical(update: Update, context):
    ...
    command = "echo {} | sudo -S grep -i 'critical' /var/log/syslog | tail -n 5".format(
        RM_PASSWORD
    )
    result = connectToHost(command)
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о последних 5 критических событий"
        )
...
def getPs(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о запущенных процессах в системе"
        )
...
def getSs(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию об используемых портах в системе"
        )
...
def getAptList(update: Update, context):
    ...
    if not query:
        logger.error(f'Команда не передана')
        reply_text = "Неверная команда. Пожалуйста, используйте:\n\n/get_apt_list all - Вывести список всех установленных пакетов\n/get_apt_list <название> - Поиск информации о конкретном пакете"
        update.message.reply_text(reply_text)
        return ConversationHandler.END
    
    if query[0] == "all":
        showAllPackages(update, context)
    else:
        searchPackage(update, context, query[0])
...
def searchPackage(update: Update, context, query):
    """Поиск информации о конкретном пакете"""
    logger.info(f"Пользователь выбрал поиск информации о пакете '{query}'")
    ...
    if result:
        ...
        update.message.reply_text(f'Не удалось найти информацию об этом пакете: {query}')
...
def getServices(update: Update, context):
    ...
    if result:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о запущенных сервисах в системе"
        )
...
def getEmails(update: Update, context):
    ...
    try:
        emails = execute_query(
            query="SELECT * FROM email_addresses;", query_type="select"
        )
        for email in emails:
        ...
        update.message.reply_text(
            "Не удалось получить информацию о хранимых в базе email адресах"
        )
...
def getPhoneNumbers(update: Update, context):
    ...
    try:
        phone_numbers = execute_query(
            query="SELECT * FROM phone_numbers;", query_type="select"
        )
        ...
        update.message.reply_text(
            "Не удалось получить информацию о хранимых в базе телефонных номеров"
        )
...
def main():
    ...
    convHandlerFindPhoneNumbers = ConversationHandler(
        ...
        states={
            ...
            SAVE_PHONE_NUMBER_STATE: [
                MessageHandler(Filters.regex(r"^yes$"), savePhoneNumber),
    ...
    convHandlerFindEmails = ConversationHandler(
        ...
        states={
            ...
            SAVE_EMAIL_ADDRESS_STATE: [
                MessageHandler(Filters.regex(r"^yes$"), saveEmailAddress),
                MessageHandler(Filters.regex(r"^no$"), declineSaving),
            ],
        },
        fallbacks=[
    ...
    convHandlerCheckPassword = ConversationHandler(
        ...
        states={
            "checkPassword": [
                MessageHandler(Filters.text & ~Filters.command, checkPassword)
            ],
        },
        fallbacks=[],
    )
    ...
    convHandlerGetDf = ConversationHandler(
        ...
        states={"getDf": [MessageHandler(Filters.text & ~Filters.command, getDf)]},
        fallbacks=[],
    )

    convHandlerGetFree = ConversationHandler(
        entry_points=[CommandHandler("get_free", getFree)],
        states={"getFree": [MessageHandler(Filters.text & ~Filters.command, getFree)]},
        fallbacks=[],
    )
    ...
    convHandlerGetW = ConversationHandler(
        ...
        states={"getW": [MessageHandler(Filters.text & ~Filters.command, getW)]},
        fallbacks=[],
    )
    ...
    convHandlerGetCritical = ConversationHandler(
        ...
        states={
            "getCritical": [
                MessageHandler(Filters.text & ~Filters.command, getCritical)
            ]
        },
        fallbacks=[],
    )
    ...
    convHandlerGetPs = ConversationHandler(
        ...
        states={"getPs": [MessageHandler(Filters.text & ~Filters.command, getPs)]},
        fallbacks=[],
    )

    convHandlerGetSs = ConversationHandler(
        entry_points=[CommandHandler("get_ss", getSs)],
        states={"getSs": [MessageHandler(Filters.text & ~Filters.command, getSs)]},
        fallbacks=[],
    )
    ...
    conv_handler_get_apt_list = ConversationHandler(
        ...
        states={
            "get_apt_list": [
                MessageHandler(Filters.text & ~Filters.command, getAptList)
            ]
        },
        fallbacks=[],
    )
    ...
    convHandlerGetServices = ConversationHandler(
        ...
        states={
            "getServices": [
                MessageHandler(Filters.text & ~Filters.command, getServices)
            ]
        },
        fallbacks=[],
    )
    ...
    convHandlerGetReplLogs = ConversationHandler(
        ...
        states={
            "getReplLogs": [
                MessageHandler(Filters.text & ~Filters.command, getReplLogs)
            ]
        },
        fallbacks=[],
    )
    ...
    convHandlerGetPhoneNumbers = ConversationHandler(
        ...
        states={
            "getPhoneNumbers": [
                MessageHandler(Filters.text & ~Filters.command, getPhoneNumbers)
            ]
        },
        fallbacks=[],
    )
